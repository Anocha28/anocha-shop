...
import {Spinner, Alert, Image, Row, Col, Button, ListGroup, Container} from 'react-bootstrap'
import Moment from 'react-moment'
import {useDispatch, useSelector} from 'react-redux'
import { getOrderDetail, deliverOrder } from '../actions/orderActions'
import { ORDER_DELIVER_DEFAULT } from '../constants/orderConstants'
...
const AdminOrderScreen = ({match, history}) => {
    ...
    const {loading: loadingDeliver, success, error: errorDeliver} = orderDeliver
    ...
    useEffect(()=>{
        ...
        return () => {
            dispatch({type: ORDER_DELIVER_DEFAULT})
        }
        
    },[dispatch, orderId, userInfo, history, success, order])
    ...
    return (
            ...
            <Row className='border-bottom pb-3'>
                ...
                <Col>
                    ...
                    {loadingDeliver ? <Spinner animation="border" variant="dark" /> :
                    errorDeliver ? <Alert variant='danger'>{errorDeliver}</Alert> : 
                    <Button 
